...
from sac_torch.model import ActorCombine, CriticCombine
...
class SAC:
    ...
    """
    def __init__(self, state_space, observation_space, action_space,
                 date=None,
                 replay_size=int(1e6),
                 gamma=0.99,
                 tau=0.05,
        ...
        rb_kwargs['env_dict']['state'] = {"shape": state_space.shape}
        rb_kwargs['env_dict']['next_state'] = {"shape": state_space.shape}
        ...
        # critic target
        self.critic_target = CriticCombine(
            state_dim=state_space.shape[0],
            obs_dim=observation_space.shape[0],
            act_dim=action_space.shape[0]
        ).to(self.device)
    ...
    def select_action(self, state, obs, evaluate=False):
        state = torch.FloatTensor(state).to(self.device).unsqueeze(0)
        obs = torch.FloatTensor(obs).to(self.device).unsqueeze(0)
        if evaluate is False:
            action, _, _ = self.actor.sample(state, obs)
        else:
            _, _, action = self.actor.sample(state, obs)
        return action.detach().cpu().numpy()[0]

    def compute_td_error(self, state, obs, act, next_state, next_obs, rew, done):
        with torch.no_grad():
            next_act, next_log_prob, _ = self.actor.sample(next_state, next_obs)
            target_q1, target_q2 = self.critic_target(next_state, next_obs, next_act)
            target_q = torch.min(target_q1, target_q2) - self.alpha * next_log_prob
            target_q = rew + ((1 - done) * self.gamma * target_q)

        current_q1, current_q2 = self.critic(state, obs, act)
    ...
    def critic_loss(self, state, obs, act, next_state, next_obs, rew, done):
        td_error1, td_error2 = self.compute_td_error(state, obs, act, next_state, next_obs, rew, done)
    ...
    def actor_alpha_loss(self, state, obs):

        act, log_prob, _ = self.actor.sample(state, obs)
    ...
    def update_critic(self, state, obs, act, next_state, next_obs, rew, done):
        critic_loss = self.critic_loss(state, obs, act, next_state, next_obs, rew, done)
    ...
    def update_actor_alpha(self, state, obs):
        actor_loss, alpha_loss = self.actor_alpha_loss(state, obs)
    ...
    def update_parameters(self, batch_size, updates):
        ...
        critic_loss = self.update_critic(state, obs, act, next_state, next_obs, rew, done)
        actor_loss, alpha_loss = self.update_actor_alpha(state, obs)
...
def eval(env, agent):
    ...
    for PosIndex in range(1, 1 + 1):
        state, obs = env.reset(PosIndex=PosIndex, random_position=False)
        done = False
        episode_reward, episode_steps = 0, 0
        while not done:
            act = agent.select_action(state, obs, evaluate=True)
            # print(act)
...
def run(batch_size=256,
    ...
    for t in trange(n_steps):
        ...
        if t < start_steps:
            ...
            act = agent.select_action(state, obs)

        next_state, next_obs, rew, done, _ = env.step(act)

        episode_rew += rew
        episode_steps += 1

        agent.rb.add(state=state, obs=obs, act=act, next_state=next_state, next_obs=next_obs, rew=rew, done=done)
