...
class CrazycarGymEnv2(gym.Env):
    ...
    def __init__(self,
        ...
        self.seed()
        # self.reset()
        observationDim = 3  # len(self.getExtendedObservation())
    ...
    def reset(self, newCarPos=None):
        ...
        if newCarPos is None:
            carPos = self._poscar.getNewPosition(random.randint(2, self._poscar.len()))
        else:
            carPos = newCarPos
        ...
        self._ballUniqueId = self._p.loadURDF(os.path.join(self._urdfRoot, "sphere2.urdf"), [ballx, bally, ballz],
                                              globalScaling=0.60)
        self._p.changeDynamics(self._ballUniqueId, -1, mass=10)
...
#        self._observation.extend([ballPosInCar[0], ballPosInCar[2]])

        posEuler = self._p.getEulerFromQuaternion(carorn)
        yaw = posEuler[2]
        self._observation.extend([carpos[0], carpos[2], yaw])
    ...
    def _reward(self):
        ...
        if (numPt > 0):
            ...
            # print("{}: {}".format(self._envStepCounter, reward))
        return reward
